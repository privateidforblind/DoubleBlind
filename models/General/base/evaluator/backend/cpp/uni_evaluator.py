...
from models.General.base.util import DataIterator
from models.General.base.util import typeassert
from .cpp_evaluator import CPPEvaluator
from models.General.base.util.cython.tools import float_type, is_ndarray
from models.General.base.util import pad_sequences
#from util.cython.arg_topk import arg_topk
import time
from models.General.base.util.tool import argmax_top_k, max_top_k
...
class UniEvaluator(CPPEvaluator):
    ...
    * **Second**, this class and its evaluation metrics can automatically fit
    ...
    def __init__(self, dataset, user_train_dict, user_test_dict, user_neg_test=None,
                 metric=None, top_k=50, batch_size=1024, num_thread=8,dump_dict=None,masked_items=None):
        ...
        self.masked_items = masked_items
        ...
        for batch_users in test_users:
            if self.user_neg_test is not None:
                ...
                for idx, user in enumerate(batch_users):
                    ...
                    dump_items = self.user_pos_dump[user]
                    dump_items = [ x for x in dump_items if not x in self.user_pos_test[user] ]
                ...
                # print("inference time: %.2f" % (inference_end_time - inference_start_time))
                # print(ranking_score.shape, len(batch_users))
                # exit()
                if not is_ndarray(ranking_score, float_type):
                    ranking_score = np.array(ranking_score, dtype=float_type)
                ...
                for idx, user in enumerate(batch_users):
                    ...
                    # dump_items = [ x for x in dump_items if not x in self.user_pos_test[user] ]
                    # 优化上一句话，用set
                    dump_items = list(set(dump_items) - set(self.user_pos_test[user]))
                    ranking_score[idx][dump_items] = -np.inf
                    # 对于self.masked_items中的item,设置为负无穷
                    if self.masked_items is not None:
                        ranking_score[idx][self.masked_items] = -np.inf
